...
项目的token是存在数据库缓存中的，有一定的有效时间，若有效时间太长，可能存在用户关闭会话后token仍然有效，存在一定安全问题，若设置的时间太短，则会存在token频繁失效的问题，为了更好的用户体验，可以设置一个合适的有效时间（Tencent是3600s），同时设置一个刷新token操作，减少要求用户频繁登陆。每次获取token时，返回客户端一个token有效时间存在客户端，客户端设置拦截器，每次请求前判断token是否过期或者将要过期，若将要过期则刷新token，若已过期则要求用户重新登陆。同时考虑到用户连续在线时常不会太长，刷新token时可以缩短token有效时间。 
